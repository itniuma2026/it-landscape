...
function sourceTitle(source, netbox) {
  if (source !== 'netbox') return '';
  const details = [];
  if (netbox?.urlSource === 'dev-default') details.push('URL dev par défaut');
  if (netbox?.tokenSource === 'dev-default') details.push('token dev par défaut');
  return details.length ? details.join(' · ') : 'Configuration NetBox explicite';
}
        ...
        <>
          <section className="business-command-center quality-command-center page-shell">
            <div className="business-command-intro">
              <span className="business-section-kicker">Vue qualité</span>
              <h1>Score global</h1>
                <h2>{quality.score}/100</h2>
                <p>
                  {quality.score >= 85
                    ? 'Fiable pour la gouvernance'
                    : 'Exploitable, corrections prioritaires'}
                </p>
            </div>
