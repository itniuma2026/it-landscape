...
const DEV_NETBOX_URL = 'http://localhost:8080';
const DEV_NETBOX_TOKEN = '0123456789abcdef0123456789abcdef01234567';
...
function isProductionRuntime() {
  const applicationEnv = process.env.IT_LANDSCAPE_ENV || process.env.APP_ENV || process.env.NODE_ENV;
  return applicationEnv === 'production';
}

function getNetboxConfig() {
  const explicitUrl = process.env.NETBOX_URL?.trim();
  const explicitToken = process.env.NETBOX_TOKEN?.trim();
  const useDevDefaults = !isProductionRuntime();
  const url = explicitUrl || (useDevDefaults ? DEV_NETBOX_URL : '');
  const token = explicitToken || (useDevDefaults ? DEV_NETBOX_TOKEN : '');

  return {
    enabled: Boolean(url && token),
    url,
    urlSource: explicitUrl ? 'env' : (url ? 'dev-default' : 'missing'),
    token,
    tokenSource: explicitToken ? 'env' : (token ? 'dev-default' : 'missing'),
  };
}
